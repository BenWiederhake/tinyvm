...
class AsmTests(unittest.TestCase):
    def test_empty(self):
        self.assertEqual(b"\x00" * SEGMENT_LENGTH, asm.compile_to_segment(""))
        self.assertEqual(b"\x00" * SEGMENT_LENGTH, asm.compile_to_segment("\n"))
    ...
    def test_hardcoded(self):
        for i, (name, asm_text, code_prefix_hex) in enumerate(ASM_TESTS):
            with self.subTest(i=i, name=name):
                ...
                self.assertTrue(len(expected_segment) <= SEGMENT_LENGTH)
                self.assertEqual(len(expected_segment) % 2, 0)
                padding = b"\x00" * (SEGMENT_LENGTH - len(expected_segment))
